...
describe('PaginagedAll', () => {
  ...
  it('finds all models', async () => {
    axiosAdapter.onGet('users?page=1').reply(200, [{ id: 1, name: 'John Doe' }, { id: 2, name: 'Jane Doe' }]);
    axiosAdapter.onGet('users?page=2').reply(200, [{ id: 3, name: 'Bob Vence' }, { id: 2, name: 'Refrigeration' }]);

    const firstPageUsers = await User.paginated(1);
    ...
    const secondPageUsers = await User.paginated(2);
    expect(secondPageUsers.length).toEqual(2);
    expect(secondPageUsers[0]).toBeInstanceOf(User);
    expect(secondPageUsers).toEqual([{ id: 3, name: 'bob vence' }, { id: 2, name: 'refrigeration' }]);
  });
});
